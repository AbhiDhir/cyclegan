...
class cyclegan(object):
    def __init__(self, sess, args):
        ...
        self.options = OPTIONS._make(
            (args.batch_size, args.fine_size, args.ngf, args.ndf, args.output_nc, args.phase == "train")
        )
    ...
    def _build_model(self):
        ...
        self.fake_A_sample = tf.compat.v1.placeholder(
            tf.float32, [None, self.image_size, self.image_size, self.input_c_dim], name="fake_A_sample"
        )
        self.fake_B_sample = tf.compat.v1.placeholder(
            tf.float32, [None, self.image_size, self.image_size, self.output_c_dim], name="fake_B_sample"
        )
        ...
        self.test_A = tf.compat.v1.placeholder(
            tf.float32, [None, self.image_size, self.image_size, self.input_c_dim], name="test_A"
        )
        self.test_B = tf.compat.v1.placeholder(
            tf.float32, [None, self.image_size, self.image_size, self.output_c_dim], name="test_B"
        )
    ...
    def train(self, args):
        ...
        self.d_optim = tf.compat.v1.train.AdamOptimizer(self.lr, beta1=args.beta1).minimize(
            self.d_loss, var_list=self.d_vars
        )
        self.g_optim = tf.compat.v1.train.AdamOptimizer(self.lr, beta1=args.beta1).minimize(
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(0, batch_idxs):
                ...
                batch_images = [
                    load_train_data(batch_file, args.load_size, args.fine_size) for batch_file in batch_files
                ]
    ...
    def test(self, args):
        ...
        for sample_file in sample_files:
            ...
            image_path = os.path.join(
                args.test_dir, "{0}_{1}".format(args.which_direction, os.path.basename(sample_file))
            )
            ...
            index.write(
                "<td><img src='%s'></td>"
                % (sample_file if os.path.isabs(sample_file) else (".." + os.path.sep + sample_file))
            )
            index.write(
                "<td><img src='%s'></td>"
                % (image_path if os.path.isabs(image_path) else (".." + os.path.sep + image_path))
            )
            index.write("</tr>")
        index.close()
